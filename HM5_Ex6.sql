...
VALUES
(1,'Петр', 'Петрович', 'Петренко', 2000, 0, '2009-11-20', NULL, NULL),
...
-- SELECT -- 
-- Используя и ShopDB получить имена покупателей и имена сотрудников у которых TotalPrice товара больше 1000

SELECT (SELECT LName FROM Employees WHERE orders.EmployeeID = Employees.EmployeeID) AS Employee,
		(SELECT LName FROM Customers WHERE orders.customerNo = customers.customerNo ) AS Customer	
	FROM Orders
    WHERE orders.OrderID IN (SELECT OrderID FROM orderdetails WHERE TotalPrice > 1000);
